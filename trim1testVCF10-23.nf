...
params.reads = params.input.fastq_path+'/19AN*R{1,2}*.fastq.gz'
...
process annotate {
    ...
    script:
        """
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar 6genes4 ${vcf_path1} > ${sample}-1.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar 6genes4 ${vcf_path2} > ${sample}-2.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar 6genes4 ${vcf_path3} > ${sample}-3.ann.vcf
        """
...
process merge {
    ...
    script:
        ...
        python $baseDir/annotate.py -r $baseDir/ref/pfalciparum/New_6_genes.fa -b $baseDir/New_6_genes.bed -o ${sample} -v1 ${vcf_path1} -v2 ${vcf_path2} -v3 ${vcf_path3} -m ${bam_path} -voi $baseDir/ref/pfalciparum/Reportable_SNPs.csv -name ${sample}
        """
}
...
process snpfilter {
    ...
    script:
        """
        python $baseDir/snpreport1-20.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample} -e1 $baseDir/candidates.xlsx -e2 $baseDir/voi2.csv 
...
process tojson {
    ...
    script:
        """
        python $baseDir/tojson.py -d1 $baseDir/$params.output.folder/forjson
        """
}