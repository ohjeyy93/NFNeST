...
for line in f3:
    ...
    if len(tempword)==4:
        ...
        if len(tempword22)==1 and len(tempword3)==2:
            ...
            #print(tempword3[1])
            #print(tempword22[0])
            DepthVal3[tempgeneval1,tempposval1]=[int(tempword3[1])/int(tempword22[0])]
        ...
        if count==1:
            ...
            if word1!="":
                ...
                for x in range(len(Genelist1)):
                    ...
                    if word1== Genelist1[x] and word== POSlist1[x] and line.startswith("#")==False and (word1,word) in DepthVal:
                        #print(word)
                        #print(word)
                        #print(word)
                        #filtercount+=1
...
result=subprocess.Popen(['samtools', 'index', args.name+ "_SR.bam"])
process=subprocess.Popen(['samtools', 'coverage', args.name+ "_SR.bam", '-r', 'mitochondrial_genome:3492-4622'], stdout=subprocess.PIPE)
stdout, stderr = process.communicate()
reader2 = csv.reader(stdout.decode('ascii').splitlines(),
                        delimiter='\t', skipinitialspace=True)
...
result=subprocess.Popen(['samtools', 'index', args.name+ "_SR.bam"])
process=subprocess.Popen(['samtools', 'coverage', args.name+ "_SR.bam"], stdout=subprocess.PIPE)
stdout, stderr = process.communicate()
reader2 = csv.reader(stdout.decode('ascii').splitlines(),
                        delimiter='\t', skipinitialspace=True)
    ...
    if line[0].startswith("#")==False:
        for word in line:
            ...
            if count==1:
                tempcovword1=word
            #print(word)
            if count==6 and float(word)>0 and (tempcovword1 not in tempcovwordlist1):
                #print(word)
                #print(word)
                #print(tempcovword1)
                tempcov1+=[word]
...
for x in wholepotlist1:
    #print(x)
    for y in depthlist1:
        if x[0] == y[0] and int(x[1])*3-3 == int(y[1]):
            #print("true")
            wildic1[x[0],x[1]]=y[2]
        if x[0] == y[0] and int(x[1])*3-2 == int(y[1]):
            wildic1[x[0],x[1]]=int(wildic1[x[0],x[1]])+int(y[2])
        if x[0] == y[0] and int(x[1])*3-1 == int(y[1]):
            wildic1[x[0],x[1]]=int(wildic1[x[0],x[1]])+int(y[2])
            #print(y)
            #print(y[2])
            #print(x[0],x[1])
            #print(dicttrans1[x[0]][int(x[1])-3])
            #print(dicttrans1[x[0]][int(x[1])-2])
            #print(dicttrans1[x[0]][int(x[1])-1])
            #print(x[1])
    if (x[0],x[1]) in wildic1:
        wildic1[x[0],x[1]]=str(int(wildic1[x[0],x[1]]/3))

posdic1={}
for x in wholepotlist1:
    posdic1[x[0],x[1]]=int(x[1])*3-1

#print(posdic1)

#print(wholepotlist1)
#print(testdicttransAA1["PfMDR1"][3])
#print(GenePosSet2)
...
for y in wholepotlist1:
    ...
    for x in range(len(combinedGenePOSlist1)):
        ...
        #print(combinedGenePOSlist1)
        if x==len(combinedGenePOSlist1)-1 and ([y[0],int(y[1])] not in combinedGenePOSlist1) and ([y[0],int(y[1])] not in combinedwildgeneAA) and (y[0],y[1]) in wildic1:
...
#print(len(filteredDescriptionlist12))
#print(len(filteredDP4list12))
#print(len(filteredADlist12))
#print(len(filteredMQRankSumlist12))
