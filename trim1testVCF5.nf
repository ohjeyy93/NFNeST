...
params.genome = "$baseDir/ref/pfalciparum/dhps_corrected.fa"
params.bed = "$baseDir/ref⁩/pfalciparum⁩/dhps_corrected.fa.bed"
...
process GenerateVCFHap {
    ...
    script:
        """
        $baseDir/gatk-4.1.9.0/gatk HaplotypeCaller -R $baseDir/ref/pfalciparum/dhps_corrected.fa -I ${bam_path} -O ${sample}-2.vcf \\
...
process annotate {
    ...
    output:
        tuple val(sample), path("${sample}-1.ann.vcf") into anno_out1
        tuple val(sample), path("${sample}-2.ann.vcf") into anno_out2
        tuple val(sample), path("${sample}-3.ann.vcf") into anno_out3

    script:
        """
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar dhps ${vcf_path1} > ${sample}-1.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar dhps ${vcf_path2} > ${sample}-2.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar dhps ${vcf_path3} > ${sample}-3.ann.vcf
        """
...
process merge {
    ...
    script:
        ...
        python $baseDir/annotate.py -r $baseDir/ref/pfalciparum/dhps_corrected.fa.fa -b $baseDir/dhps_corrected.fa.bed -o ${sample} -v1 ${vcf_path1} -v2 ${vcf_path2} -v3 ${vcf_path3} -m ${bam_path} -voi $baseDir/ref/pfalciparum/Reportable_SNPs.csv -name ${sample}
        """
}
...
process extract {
    ...
    script:
    """
    java -jar $baseDir/snpEFF/SnpSift.jar extractFields ${vcf_path1} CHROM POS REF ALT VARTYPE Confidence Sources "ANN[*].EFFECT" "ANN[*].HGVS_C" "ANN[*].HGVS_P" > final_${vcf_path1}ext.vcf
    """
}
...
/* process extract {
    publishDir "$params.output.folder/final_vcf_SNP/${sample}", mode : "copy"
    input:
        set val(sample), path(vcf_path1) from merge_out
    output:
        tuple val(sample), path("final_${sample}_SNP.vcf") into SNP_out
    script:
        """
        java -jar $baseDir/snpEFF/SnpSift.jar extractFields ${vcf_path1} > final_${sample}_SNP.vcf
        """
...
/* process vartpype {
    ...
    input:
        set val(sample), path(vcf_path1) from extract_out

    output:
        tuple val(sample), path("{sample}.vartype.vcf") into vartype_out

    script:
        """
        java -jar $baseDir/snpEFF/SnpSift.jar varType ${vcf_path1} > {sample}.vartype.vcf
        """
} */
...
/*process filter {
    ...
    script:
        """
        cat ${vcf_path1} | java -jar $baseDir/snpEFF/SnpSift.jar filter " ( VARTYPE = 'SNP' )" > ${sample}_filtered.vcf
        """
}*/