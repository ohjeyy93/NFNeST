...
process combineFastq {
    ...
    script:
        """
        zless *R1*.fastq.gz > ${pair_id}_R1.fastq
        zless *R2*.fastq.gz > ${pair_id}_R2.fastq
        """
}
...
process trimFastq {
    ...
    script:
        """
        bbduk.sh -Xmx1g k=27 hdist=1 edist=0 ktrim=l in=${read_one} in2=${read_two} \\
        out=${sample}_trimmed_R1.fastq ref=${fas} mink=4 \\
        qtrim=rl minlength=50 trimq=30 qin=33 overwrite=true \\
...
process extract {
    ...
    script:
    """
    java -Xmx8g -jar $baseDir/snpEff/SnpSift.jar extractFields ${vcf_path1} CHROM POS REF ALT VARTYPE Confidence Sources DP4 GEN[*].AD "ANN[*].EFFECT" "ANN[*].HGVS_C" "ANN[*].HGVS_P" > final_${vcf_path1}ext.vcf
    """
}
...
process snpfilter {
    ...
    script:
        """
        python $baseDir/snpreport1-53-2.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample} -e1 $baseDir/candidates.xlsx -e2 $baseDir/voinew2.csv -f1 $baseDir/New_6_genes.fa -b2 $baseDir/mdr.bed
        mv $logfile "nextflowlog.txt"
        """
}

process lowestCoverage {

    publishDir "$params.output.folder/lowestCoverage/", pattern: "*.csv", mode : "copy"
    publishDir "$params.output.folder/snpfilter2/"
    input:
        set val(sample), path("${sample}.csv") from snpfilter_out1
    output:
        tuple val(sample), path("lowcov1.csv") into lowcov_out
    script:
        """
        python $baseDir/lowcov1.py -d1 $baseDir/$params.output.folder/forjson -d2 $baseDir/fq/AngolaControls
        """
}
...
process snpfilter2 {
    ...
    input:
        set val(sample), path("${sample}.csv") from lowcov_out
    script:
        """
        python $baseDir/snptranslate2.py -d1 $baseDir/$params.output.folder/forjson -l1 $baseDir/$params.output.folder/lowestCoverage/lowcov1.csv -d2 $baseDir/$params.output.folder/snpfilter2
        """
}
...
process tojson {
    ...
    script:
        """
        python $baseDir/tojson3.py -d1 $baseDir/$params.output.folder/forjson
        """
}
