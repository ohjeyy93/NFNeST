...
process repair {
    ...
    output:
        tuple val(sample), path("${sample}_repaired_R1.fastq"), path("${sample}_repaired_R2.fastq"), path("${sample}_singleone.fastq") into repair_out

    script:
        """          
        repair.sh in1=${read_one} in2=${read_two} out1=${sample}_repaired_R1.fastq out2=${sample}_repaired_R2.fastq outs=${sample}_singleone.fastq repair  
        """
}
...
process trimFastq {
    ...
    input:
        set val(sample), path(read_one), path(read_two) from repair_out
        path fas from params.fas
...
process extract {
    ...
    script:
    """
    java -Xmx8g -jar $baseDir/snpEff/SnpSift.jar extractFields ${vcf_path1} CHROM POS REF ALT VARTYPE Confidence Sources DP AD GEN[*].AD "ANN[*].EFFECT" "ANN[*].HGVS_C" "ANN[*].HGVS_P" > final_${vcf_path1}ext.vcf
    """
}
...
process snpfilter {
    ...
    script:
        """
        python $baseDir/snpreport1-54-AG.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample} -e1 $baseDir/candidates.xlsx -e2 $baseDir/voinew3.csv -f1 $baseDir/New_6_genes.fa -b2 $baseDir/mdr.bed
