...
params.genome = "$baseDir/ref/pfalciparum/PF3D7_dhps.fa"
params.bed = "$baseDir/ref⁩/pfalciparum⁩/PF3D7_dhps.bed"
...
process GenerateVCFHap {
    ...
    script:
        """
        $baseDir/gatk-4.1.9.0/gatk HaplotypeCaller -R $baseDir/ref/pfalciparum/PF3D7_dhps.fa -I ${bam_path} -O ${sample}-2.vcf \\
...
process annotate {
    ...
    script:
        """
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar PF3D7_dhps ${vcf_path1} > ${sample}-1.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar PF3D7_dhps ${vcf_path2} > ${sample}-2.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar PF3D7_dhps ${vcf_path3} > ${sample}-3.ann.vcf
        """
...
process merge {
    ...
    script:
        ...
        python $baseDir/annotate.py -r $baseDir/ref/pfalciparum/PF3D7_dhps.fa -b $baseDir/PF3D7_dhps.bed -o ${sample} -v1 ${vcf_path1} -v2 ${vcf_path2} -v3 ${vcf_path3} -m ${bam_path} -voi $baseDir/ref/pfalciparum/Reportable_SNPs.csv -name ${sample}
        """
}
...
process extract {
    ...
    script:
    """
    java -jar $baseDir/snpEFF/SnpSift.jar extractFields ${vcf_path1} CHROM POS REF ALT VARTYPE Confidence "ANN[*].EFFECT" "ANN[*].HGVS_C" "ANN[*].HGVS_P" > final_${vcf_path1}ext.vcf
    """
}
