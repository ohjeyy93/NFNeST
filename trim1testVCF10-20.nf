...
process annotate {
    ...
    script:
        """
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar 6genes3 ${vcf_path1} > ${sample}-1.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar 6genes3 ${vcf_path2} > ${sample}-2.ann.vcf
        java -Xmx8g -jar $baseDir/snpEff/snpEff.jar 6genes3 ${vcf_path3} > ${sample}-3.ann.vcf
        """
...
process snpfilter {
    ...
    script:
        """
        python $baseDir/snpreport1-18.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample} -e1 $baseDir/candidates.xlsx -e2 $baseDir/voi2.csv 
