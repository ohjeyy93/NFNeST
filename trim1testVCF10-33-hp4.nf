...
process GenerateVCF {
    ...
    publishDir "$params.output.folder/GenerateVCFSam/${sample}", pattern: "*.bam", mode : "copy"
    input:
        set val(sample), path(bam_path) from postal_out
        path genome from params.genome

    output:
        tuple val(sample), path("${sample}-1.vcf"), path("${sample}-2.vcf"), path("${sample}-3.vcf"), path("${sample}.bam") into vcf_out1

    script:
        """
        bcftools mpileup -B -Q 0 -q 0 -d 0 -A -B -f ${genome} ${bam_path}  > ${sample}.mpileup
        bcftools call -vm ${sample}.mpileup > ${sample}-1.vcf
        $baseDir/gatk-4.1.9.0/gatk HaplotypeCaller -R $baseDir/New_6_genes.fa -I ${bam_path} -O ${sample}-2.vcf --base-quality-score-threshold 6 --max-reads-per-alignment-start 0 --min-base-quality-score 0 --standard-min-confidence-threshold-for-calling 0 --max-mnp-distance 10 --active-probability-threshold 0 --force-active --force-call-filtered-alleles --debug --linked-de-bruijn-graph --recover-all-dangling-branches --min-pruning 0 -bamout ${sample}.bam
        freebayes -f ${genome} -q 0 -m 0 -Q 0 -F 0.01 -C 0 -W 1,8 --genotyping-max-banddepth 30 --report-all-haplotype-alleles  --report-monomorphic ${bam_path} > ${sample}-3.vcf
        """
}
