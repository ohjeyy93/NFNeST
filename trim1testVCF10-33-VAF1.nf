...
process combineFastq {
    ...
    script:
        """
        zless *_1*.fastq.gz > ${pair_id}_1.fastq
        zless *_2*.fastq.gz > ${pair_id}_2.fastq
        """
}
...
process snpfilter {
    ...
    script:
        """
        python $baseDir/snpreport1-54-VAF.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample} -e1 $baseDir/candidates.xlsx -e2 $baseDir/voinew3.csv -f1 $baseDir/New_6_genes.fa -b2 $baseDir/mdr.bed
