...
def main(r,b,o,v1,v2,v3,m,voi,n):
    ...
    annotate = Annotate()
    gvcf_path = annotate.getAnnotation(bed_path, vcf_path2, ref_path, out_path, bam_path)
    vcf_path = annotate.getAnnotation(bed_path, vcf_path1, ref_path, out_path, bam_path)
    fvcf_path = annotate.getAnnotation(bed_path, vcf_path3, ref_path, out_path, bam_path)
    ...
    with open("final_"+sam_name+".vcf", "a") as outF:
      print(('Total variants : {0}; Verified calls : {1}; Exonic : {2}; Intronic : {3}; Synonymous : {4}; Non Synonymous : {5}; Transition : {6}; Transversion : {7}'.format(
                        var_sum[0], var_sum[1], var_sum[2], var_sum[3], var_sum[4], var_sum[5], var_sum[6], var_sum[7])), file=outF)
