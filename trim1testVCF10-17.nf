...
process snpfilter {
    ...
    script:
        """
        python $baseDir/snpreport1-17.py -v1 ${vcf_path2} -v2 ${vcf_path1} -b1 ${bam_path1} -o1 ${sample} -e1 $baseDir/candidates.xlsx -e2 $baseDir/voi2.csv 
...
process tojson {
    publishDir "$params.output.folder/tojson/"
    errorStrategy 'ignore'
    input:
